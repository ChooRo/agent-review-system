...
def test_start_persists_mock_findings_and_events(tmp_path, monkeypatch) -> None:
    ...
    started = client.post(f"/api/v1/projects/{project['id']}/procurement-review-tasks/{task['id']}/start", headers=headers)
    assert started.status_code == 200 and started.json()["finding_summary"]["total"] > 0
    ...
    assert findings[0]["source"] and events
...
def test_procurement_review_end_to_end_recheck_flow(tmp_path, monkeypatch) -> None:
    ...
    started = client.post(f"/api/v1/projects/{project['id']}/procurement-review-tasks/{task['id']}/start", headers=operator).json()
    assert started["finding_summary"]["total"] > 0 and started["progress"] == 100 and started["task_role"] == "operator"
    ...
    recheck = client.get(f"/api/v1/projects/{project['id']}/procurement-review-tasks/{task['id']}", headers=primary).json()
    ...
    completed = client.get(f"/api/v1/projects/{project['id']}/procurement-review-tasks/{task['id']}", headers=primary).json()
