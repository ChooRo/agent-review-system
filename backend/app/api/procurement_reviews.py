...
from app.schemas.procurement_review import CollaborativeComment, EventOut, FindingOut, LegalApplicabilityConfirmation, OperatorDisposition, PrimaryDecision, ProjectCreate, ProjectOut, ProjectUpdate, TaskCreate, TaskOut
...
@router.post("/{project_id}/procurement-review-tasks/{task_id}/rectification-document", response_model=TaskOut)
def upload_rectification_document(project_id: str, task_id: str, user: CurrentUser, service: Service, file: UploadFile = File(...)): return service.upload_rectification(project_id, task_id, file, user)
...
@router.put("/{project_id}/procurement-review-tasks/{task_id}/legal-applicability/{document_key}/confirmation", response_model=TaskOut)
def confirm_legal_applicability(project_id: str, task_id: str, document_key: str, payload: LegalApplicabilityConfirmation, user: CurrentUser, service: Service): return service.confirm_legal_applicability(project_id, task_id, document_key, payload.model_dump(), user)
...
@router.post("/{project_id}/procurement-review-tasks/{task_id}/lock-final", response_model=TaskOut)
def lock_final(project_id: str, task_id: str, user: CurrentUser, service: Service): return service.lock_final(project_id, task_id, user)
