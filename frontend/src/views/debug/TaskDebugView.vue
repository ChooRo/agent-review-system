...
function inputBlocks(call: any) { return list(callInput(call)?.blocks) }
  ...
  <div class="debug-page">
    ...
    <main v-else>
      <aside class="process">
        ...
        <button v-for="(stage, index) in stages" :key="stage.key" :class="{ active: active?.key === stage.key, ai: stage.kind === 'ai' }" @click="selected = stage.key">
      ...
      <section v-if="active" class="content">
        ...
        <template v-else-if="active.key === 'logical_units'">
          ...
          <article v-for="unit in logicalUnits" :key="unit.unit_id" class="card unit-content"><div class="unit-head"><div><span>{{ unitName(unit.unit_type) }}</span><h3>{{ unitTitle(unit) }}</h3></div><small>{{ unit.unit_id }}</small></div><p class="meta">第 {{ unit.page_range?.[0] || '—' }}–{{ unit.page_range?.[1] || '—' }} 页 · {{ unit.token_estimate }} Token · {{ unit.primary_block_ids?.length || 0 }} 个主要 Block</p><div class="unit-blocks"><article v-for="block in (expandedUnit === unit.unit_id ? primaryBlocks(unit) : primaryBlocks(unit).slice(0, 3))" :key="`${block.block_id}-${block.role}`"><div><b>{{ blockTypeLabel(block.type) }} · 第 {{ block.page || '—' }} 页</b><small>{{ block.block_id }}</small></div><div v-if="block.table_html" class="table-shell" v-html="safeTableHtml(block.table_html)"></div><p v-else>{{ cleanText(block.text) }}</p></article></div><button v-if="primaryBlocks(unit).length > 3" class="content-toggle" @click="expandedUnit = expandedUnit === unit.unit_id ? '' : unit.unit_id">{{ expandedUnit === unit.unit_id ? '收起内容' : `查看全部 ${primaryBlocks(unit).length} 个 Block` }}</button></article>
        </template>
        ...
        <template v-else-if="active.key === 'review_batches'">
          ...
          <article v-for="batch in active.batches" :key="batch.batch_no" class="card batch-content"><div class="batch-head"><div><span>BATCH {{ String(batch.batch_no).padStart(2, '0') }}</span><h3>第 {{ batch.batch_no }} 批 · {{ batch.purpose || '采购文件理解' }}</h3></div><b>{{ batch.primary_block_count ?? batchBlocks(batch).length }} Blocks</b></div><p class="meta">{{ batch.token_estimate ?? batch.character_count ?? 0 }} {{ batch.token_estimate != null ? 'Token' : '字符' }} · 预计候选 {{ batch.candidate_estimate ?? '—' }} 条 · 表格业务行 {{ batch.table_row_count ?? 0 }} 行 · {{ batch.coverage_strategy || '完整逻辑单元装批' }}</p><div class="batch-blocks"><article v-for="block in (expandedBatch === batch.batch_no ? batchBlocks(batch) : batchBlocks(batch).slice(0, 8))" :key="`${block.block_id}-${block.role}-${block.text}`"><div><b>{{ blockTypeLabel(block.type) }} · 第 {{ block.page || '—' }} 页</b><small>{{ block.block_id }}</small></div><div v-if="block.table_html" class="table-shell" v-html="safeTableHtml(block.table_html)"></div><p v-else>{{ cleanText(block.text) }}</p></article></div><button v-if="batchBlocks(batch).length > 8" class="content-toggle" @click="toggleBatch(batch.batch_no)">{{ expandedBatch === batch.batch_no ? '收起批次内容' : `查看全部 ${batchBlocks(batch).length} 个输入 Block` }}</button></article>
        </template>
...
.table-shell{max-height:420px;overflow:auto;margin-top:9px;border:1px solid #ddd8cf;border-radius:7px;background:#fff}.table-shell :deep(table){width:100%;border-collapse:collapse;font-size:12px;line-height:1.55}.table-shell :deep(th),.table-shell :deep(td){min-width:96px;padding:8px 10px;border:1px solid #e3ded5;vertical-align:top;text-align:left}.table-shell :deep(th),.table-shell :deep(tr:first-child td){position:sticky;top:0;z-index:1;background:#f0ede5;color:#34413b;font-weight:600}.table-shell :deep(tr:nth-child(even) td){background:#fbfaf6}
</style>
